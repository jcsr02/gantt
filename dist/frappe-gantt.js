...
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/padStart
    ...
    class Arrow {
        ...
        calculate_path() {
            ...
            this.path = `
            M ${start_x+this.from_task.$bar.getWidth()/2} ${start_y-10}
            a 5 5 0 0 1 5 5
            M ${start_x+this.from_task.$bar.getWidth()/2+5} ${start_y-5}
            v 10
            a 5 5 0 0 1 -5 5
            H ${start_x+this.from_task.$bar.getWidth()/2 - 20}
            a 5 5 0 0 0 -5 5
            V ${offset}
            a 5 5 0 0 0 5 5
            L ${end_x} ${end_y}
            ...
            `;
            /*
            M ${start_x+this.from_task.$bar.getWidth()/2-2.5} ${start_y-10}
                    l 10 0
                    V ${start_y+9}
                    M ${start_x+(this.from_task.$bar.getWidth()/2)+8.3} ${start_y+9}
                    l -${this.from_task.$bar.getWidth()/2+8.3} 0
                    M ${start_x} ${start_y+8.3}
                    V ${offset}
            ...
            if (
                ...
                if (this.to_task.period.periods) {
                    this.path = `
                M ${start_x+this.from_task.$bar.getWidth()/2} ${start_y-10}
                a 5 5 0 0 1 5 5
                M ${start_x+this.from_task.$bar.getWidth()/2+5} ${start_y-5}
        ...
        calculate_path1() {
            ...
            const start_x_modified = this.from_task.$bar.getX() + this.from_task.$bar.getWidth();
            ...
            const clockwise = from_is_below_to ? 1 : 0;
            const curve_y = from_is_below_to ? -curve : curve;
            let offset = from_is_below_to
                ? end_y + this.gantt.options.arrow_curve
                : end_y - this.gantt.options.arrow_curve;

            if (!from_is_below_to) {
                offset = offset - (this.gantt.options.arrow_curve * 2);
            }

            this.path = `
            M ${start_x_modified} ${start_y}
            V ${offset}
            a ${curve} ${curve} 0 0 ${clockwise} ${curve} ${curve_y}
            L ${end_x} ${end_y}
            m 15 -15
    ...
    class Gantt {
        ...
        setup_gantt_dates() {
            ...
            // Check if end date is configured via options
        ...
        make_dates() {
            for (let date of this.get_dates_to_draw()) {
                ...
                if (date.upper_text) {
                    ...
                    const $upper_text1 = createSVG('text', {
                        x: date.upper_x-530,
                        y: date.upper_y,
                        innerHTML: date.upper_text,
                        class: 'upper-text',
                    ...
                    const $upper_text2 = createSVG('text', {
                        x: date.upper_x+530,
                        y: date.upper_y,
                        innerHTML: date.upper_text,
                        class: 'upper-text',
